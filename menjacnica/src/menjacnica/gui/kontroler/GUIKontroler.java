...
public class GUIKontroler {
	...
	public static void prikaziObrisiKursGUI() {
		...
		if (gp.getTable().getSelectedRow() != -1) {
			MenjacnicaTableModel model = (MenjacnicaTableModel) (gp.getTable().getModel());
			ObrisiKursGUI prozor = new ObrisiKursGUI(gp, model.vratiValutu(gp.getTable().getSelectedRow()));
			prozor.setLocationRelativeTo(gp);
			prozor.setVisible(true);
		}
	}
	...
	public static void prikaziIzvrsiZamenuGUI() {
		if (gp.getTable().getSelectedRow() != -1) {
			MenjacnicaTableModel model = (MenjacnicaTableModel) (gp.getTable().getModel());
			IzvrsiZamenuGUI prozor = new IzvrsiZamenuGUI(gp, model.vratiValutu(gp.getTable().getSelectedRow()));
			prozor.setLocationRelativeTo(gp);
			prozor.setVisible(true);
		}
	}
	...
	public static double izvrsiZamenu(Valuta valuta, boolean isSelected, double iznos,
			JTextField textFieldKonacniIznos) {
		double konacniIznos=0;
		try {
			konacniIznos = menjacnica.izvrsiTransakciju(valuta, isSelected, iznos);
